...
draw_cell(int m, int n)
{
  ...
  if ( (biginfo_enabled != FALSE) && (active_marker >= 0) )
  {
    ...
    format_smith_value(buf, sizeof(buf), coeff, frequencies[ markers[active_marker].index ]);
    cell_drawstring_size(w, h, buf, cxpos, cypos+=30, 0xffff, 0x0000, 3);
...
#define MARKER_Y_DELTA 10
  ...
  if (active_marker != -1 && previous_marker != -1 && uistat.current_trace != -1) 
  {
    ...
    for (mk = 0; mk < MARKERS_MAX; mk++) 
    {
      ...
      if (uistat.marker_delta && mk != active_marker) 
      {
        ...
        frequency_string_short(buf, sizeof buf, freq, C_DELTA);
      } 
      else 
      {
    ...
    if (!uistat.marker_delta && previous_marker >= 0 && active_marker != previous_marker && markers[previous_marker].enabled) 
    {
      ...
      chsnprintf(buf, sizeof buf, "%c%d:", C_DELTA, previous_marker+1);
  ...
  if (electrical_delay != 0) 
  {
    ...
    xpos += slen;
    string_value_with_prefix(buf, sizeof buf, electrical_delay * 1e-12, 's');
    slen = cell_drawstring_8x8_var(w, h, buf, xpos, ypos, 0xffff, FALSE);
    xpos += slen + 8;
